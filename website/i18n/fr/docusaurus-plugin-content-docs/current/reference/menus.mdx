...
```go title="Package: github.com/josStorer/wails/v2/pkg/menu"
type Menu struct {
    Items []*MenuItem
}
```
...
Les accélérateurs (parfois appelés raccourcis clavier) définissent une liaison entre une clé et un élément du menu. Wails définit un accélérateur comme une combinaison ou une clé + [modificateur](#modifier). Ils sont disponibles dans le paquet `"github.com/josStorer/wails/v2/pkg/menu/keys"`.

Exemple:

```go title="Package: github.com/josStorer/wails/v2/pkg/menu/keys"
